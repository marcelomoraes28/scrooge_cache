...
@memcache_client.gentlemen_cache(namespace='dict_delay_m', expiration_time=2)
...
@memcache_client.gentlemen_cache(namespace='obj_delay_m', expiration_time=2)
def m_obj_delay(delay_time=5):
    return Test()


@memcache_client.gentlemen_cache(namespace='str_delay_m', expiration_time=2)
def m_str_delay(delay_time=1):
    return 'foo-bar'


@memcache_client.gentlemen_cache(namespace='int_delay_m', expiration_time=2)
def m_int_delay(delay_time=2):
    return 9


@memcache_client.gentlemen_cache(namespace='float_delay_m', expiration_time=2)
def m_float_delay(delay_time=3):
    return 1.4


@memcache_client.gentlemen_cache(namespace='list_delay_m', expiration_time=2)
def m_list_delay(delay_time=4):
    return ['foo', 'bar']


@memcache_client.gentlemen_cache(namespace='tuple_delay_m', expiration_time=2)
def m_tuple_delay(delay_time=5):
    return (1, 2)


@memcache_client.gentlemen_cache(namespace='no_cache_m', expiration_time=10)
